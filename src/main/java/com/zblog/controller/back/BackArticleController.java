...
public class BackArticleController extends BaseController {
    ...
    public String addArticle(@RequestBody String bodyString) {
        ...
        try {
            ...
            // 参数正常
            if (!article.getTitle().isEmpty() && !article.getContent().isEmpty() && article.getCategory() != null) {
                // 如果文章添加ok,继续添加文章标签
                if (articleService.addArticle(article)) {
                    List<Integer> tags = (List<Integer>) JSONUtils.String2Object(json.get("tag").toString(),
                            List.class);
            ...
            LOGGER.warn("ArticleController.addArticle() 参数异常:[{}]", JSON.toJSONString(article));
        } catch (Exception e) {
    ...
    public ModelAndView initEditPage(Article article) {
        ...
        response.addObject("tagJson", articleDto == null ? "" : JSONUtils.toJSONString(articleDto.getTags()));
    ...
    public ModelAndView searchByKey(String key) {
        ...
        try {
            ...
            LOGGER.error("ArticleController." + "searchByKey();", e.getMessage());
        }
