...
public class ArticleCommentController extends BaseController {
	...
	@RequestMapping("/add")
	@ResponseBody
	public String addArticleComment(ArticleComment comment, HttpServletRequest request) {
		...
		try {
			...
			if (articleCommentService.addComment(comment)) {
				...
				String readUrl = "";
				if (!domain.equals("")) {
					readUrl = "http://" + domain + "/article/read.htm?id=" + comment.getArticleId();
				} else {
					readUrl = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
							+ request.getContextPath() + "/article/read.htm?id=" + comment.getArticleId();
				}
	...
	public ModelAndView getArticleComments(Integer articleId, Page page) {
		...
		response.addObject("comments", comments == null ? "" : comments.getList());
		response.addObject("page", comments == null ? "" : comments.getPage());
		return response;
	}
